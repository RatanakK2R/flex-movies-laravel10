...
@section('content')
	...
	<section class="section section--head section--head-fixed section--gradient section--details-bg">
		<div class="section__bg" data-bg="{{ asset('storage/cover_images/' . $movie->cover_image) }}"></div>
		<div class="container">
			<!-- article -->
			<div class="article">
				<div class="row">
					<div class="col-12 col-xl-8">
						<!-- trailer -->
						<a href="{{ asset('storage/videos/' . $movie->video_url) }}" class="article__trailer open-video">
						...
						<div class="article__content">
							...
							<ul class="list">
                                ...
                                @foreach($categories as $category)
                                    <li>{{ ucwords(strtolower($category->name)) }}</li>
                                @endforeach
								<li>{{ $movie->release_year }}</li>
                                <li>{{ $movie->formatted_duration }}</li>
					...
					<!-- video player -->
					<div class="col-12 col-xl-8">
						<video controls crossorigin playsinline poster="{{ asset('storage/cover_images/' . $movie->cover_image) }}" id="player">
							<!-- Video files -->
							<source src="{{ asset('storage/videos/' . $movie->video_url) }}" type="video/mp4" size="576">
							<source src="{{ asset('storage/videos/' . $movie->video_url) }}" type="video/mp4" size="720">
							<source src="{{ asset('storage/videos/' . $movie->video_url) }}" type="video/mp4" size="1080">
							...
							<a href="{{ asset('storage/videos/' . $movie->video_url) }}" download>Download</a>
						</video>
						...
						<div class="article__actions article__actions--details">
							<div class="article__download">
								...
								Download:
								<a href="{{ asset('storage/videos/' . $movie->video_url) }}" download="{{ asset('storage/videos/' . $movie->video_url) }}">480p</a>
								<a href="{{ asset('storage/videos/' . $movie->video_url) }}" download="{{ asset('storage/videos/' . $movie->video_url) }}">720p</a>
								<a href="{{ asset('storage/videos/' . $movie->video_url) }}" download="{{ asset('storage/videos/' . $movie->video_url) }}">1080p</a>
								<a href="{{ asset('storage/videos/' . $movie->video_url) }}" download="{{ asset('storage/videos/' . $movie->video_url) }}">4k</a>
							</div>
						...
						<div class="series-wrap">
							...
							<div class="section__carousel-wrap">
								<div class="section__series owl-carousel" id="series">
									...
									<div class="series" >
										<a href="{{route('detail', ['id' => $movie->id]) }}" class="series__cover" style="width: 218px; height: 180px">
											<img src="{{ asset('storage/cover_images/' . $movie->cover_image) }}" alt="{{ asset('storage/cover_images/' . $movie->cover_image) }}" style="bottom: 45px;">
											<span>
												<svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M11 1C16.5228 1 21 5.47716 21 11C21 16.5228 16.5228 21 11 21C5.47716 21 1 16.5228 1 11C1 5.47716 5.47716 1 11 1Z" stroke-linecap="round" stroke-linejoin="round"/><path fill-rule="evenodd" clip-rule="evenodd" d="M14.0501 11.4669C13.3211 12.2529 11.3371 13.5829 10.3221 14.0099C10.1601 14.0779 9.74711 14.2219 9.65811 14.2239C9.46911 14.2299 9.28711 14.1239 9.19911 13.9539C9.16511 13.8879 9.06511 13.4569 9.03311 13.2649C8.93811 12.6809 8.88911 11.7739 8.89011 10.8619C8.88911 9.90489 8.94211 8.95489 9.04811 8.37689C9.07611 8.22089 9.15811 7.86189 9.18211 7.80389C9.22711 7.69589 9.30911 7.61089 9.40811 7.55789C9.48411 7.51689 9.57111 7.49489 9.65811 7.49789C9.74711 7.49989 10.1091 7.62689 10.2331 7.67589C11.2111 8.05589 13.2801 9.43389 14.0401 10.2439C14.1081 10.3169 14.2951 10.5129 14.3261 10.5529C14.3971 10.6429 14.4321 10.7519 14.4321 10.8619C14.4321 10.9639 14.4011 11.0679 14.3371 11.1549C14.3041 11.1999 14.1131 11.3999 14.0501 11.4669Z" stroke-linecap="round" stroke-linejoin="round"/></svg> {{ $movie->formatted_duration }}
											</span>
										</a>
										<h3 class="series__title"><a href="{{route('detail', ['id' => $movie->id]) }}">{{$movie->title}}</a></h3>
									</div>
                                    @endforeach
								</div>
					...
					<div class="col-12 col-xl-8">
						...
						<div class="categories">
							...
							@foreach ($categories as $category)
							<a href="{{route('detail', ['id' => $movie->id]) }}" class="categories__item">
								{{ $category->name }}
							</a>
							@endforeach
						</div>
						...
						<div class="comments">
							...
							<div class="tab-content">
												...
												<div class="comments__rate">
													<button type="button">
														<svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
															<path d="M11 7.3273V14.6537" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M14.6667 10.9905H7.33333" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M15.6857 1H6.31429C3.04762 1 1 3.31208 1 6.58516V15.4148C1 18.6879 3.0381 21 6.31429 21H15.6857C18.9619 21 21 18.6879 21 15.4148V6.58516C21 3.31208 18.9619 1 15.6857 1Z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
														</svg> {{ $comment->likes }}
													</button>

													<button type="button">{{ $comment->dislikes }} <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14.6667 10.9905H7.33333" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M15.6857 1H6.31429C3.04762 1 1 3.31208 1 6.58516V15.4148C1 18.6879 3.0381 21 6.31429 21H15.6857C18.9619 21 21 18.6879 21 15.4148V6.58516C21 3.31208 18.9619 1 15.6857 1Z" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></button>
												</div>
								...
								<div class="tab-pane fade" id="tab-2" role="tabpanel">
										...
										@foreach ($reviews as $review)
											<li class="reviews__item">
												<div class="reviews__autor">
													<img class="reviews__avatar" src="{{asset('assets/img/avatar.svg')}}" alt="">
													<span class="reviews__name">{{ $review->title }}</span>
													<span class="reviews__time">{{ $review->created_at->format('d.m.Y, H:i') }} by {{ $review->user->name }}</span>
									...
									<form action="{{ route('reviews.store') }}" method="POST" class="reviews__form">
										@csrf
										<input type="hidden" name="movie_id" value="{{ $movie->id }}">
										<div class="row">
											...
											<div class="col-12 col-md-3 col-lg-2 col-xl-3">
												<div class="sign__group">
													<select name="rating" class="sign__select" required>
											...
											<div class="col-12">
												<div class="sign__group">
													<textarea id="text2" name="text" class="sign__textarea" placeholder="Add review" required></textarea>
												</div>
											</div>
											<div class="col-12">
												<button type="submit" class="sign__btn">Send</button>
                                    ...
                                    {{ $reviews->appends(['comment_page' => $comments->currentPage()])->links('pagination.review') }}
								</div>
						...
						<div class="sidebar sidebar--mt">
							...
							<div class="row">
								<div class="col-12">
									<form action="#" class="subscribe">
										...
										<h4 class="subscribe__title">Feadback</h4>
							...
							<div class="row row--grid">
								...
								<div class="col-6 col-sm-4 col-md-3 col-xl-6">
									<div class="card">
										<a href="details.html" class="card__cover" style="width: 160px; height: 237px;">
											<img src="{{ asset('storage/cover_images/' . $movie->cover_image) }}" alt="">
						...
						<div class="section__carousel owl-carousel" id="similar">
							<div class="card">
								<a href="details.html" class="card__cover">
									<img src="{{asset('assets/img/card/6.png')}}" alt="">
									<svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M11 1C16.5228 1 21 5.47716 21 11C21 16.5228 16.5228 21 11 21C5.47716 21 1 16.5228 1 11C1 5.47716 5.47716 1 11 1Z" stroke-linecap="round" stroke-linejoin="round"/><path fill-rule="evenodd" clip-rule="evenodd" d="M14.0501 11.4669C13.3211 12.2529 11.3371 13.5829 10.3221 14.0099C10.1601 14.0779 9.74711 14.2219 9.65811 14.2239C9.46911 14.2299 9.28711 14.1239 9.19911 13.9539C9.16511 13.8879 9.06511 13.4569 9.03311 13.2649C8.93811 12.6809 8.88911 11.7739 8.89011 10.8619C8.88911 9.90489 8.94211 8.95489 9.04811 8.37689C9.07611 8.22089 9.15811 7.86189 9.18211 7.80389C9.22711 7.69589 9.30911 7.61089 9.40811 7.55789C9.48411 7.51689 9.57111 7.49489 9.65811 7.49789C9.74711 7.49989 10.1091 7.62689 10.2331 7.67589C11.2111 8.05589 13.2801 9.43389 14.0401 10.2439C14.1081 10.3169 14.2951 10.5129 14.3261 10.5529C14.3971 10.6429 14.4321 10.7519 14.4321 10.8619C14.4321 10.9639 14.4011 11.0679 14.3371 11.1549C14.3041 11.1999 14.1131 11.3999 14.0501 11.4669Z" stroke-linecap="round" stroke-linejoin="round"/></svg>
								</a>
								<button class="card__add" type="button"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16,2H8A3,3,0,0,0,5,5V21a1,1,0,0,0,.5.87,1,1,0,0,0,1,0L12,18.69l5.5,3.18A1,1,0,0,0,18,22a1,1,0,0,0,.5-.13A1,1,0,0,0,19,21V5A3,3,0,0,0,16,2Zm1,17.27-4.5-2.6a1,1,0,0,0-1,0L7,19.27V5A1,1,0,0,1,8,4h8a1,1,0,0,1,1,1Z"/></svg></button>
								<span class="card__rating"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M22,9.67A1,1,0,0,0,21.14,9l-5.69-.83L12.9,3a1,1,0,0,0-1.8,0L8.55,8.16,2.86,9a1,1,0,0,0-.81.68,1,1,0,0,0,.25,1l4.13,4-1,5.68A1,1,0,0,0,6.9,21.44L12,18.77l5.1,2.67a.93.93,0,0,0,.46.12,1,1,0,0,0,.59-.19,1,1,0,0,0,.4-1l-1-5.68,4.13-4A1,1,0,0,0,22,9.67Zm-6.15,4a1,1,0,0,0-.29.88l.72,4.2-3.76-2a1.06,1.06,0,0,0-.94,0l-3.76,2,.72-4.2a1,1,0,0,0-.29-.88l-3-3,4.21-.61a1,1,0,0,0,.76-.55L12,5.7l1.88,3.82a1,1,0,0,0,.76.55l4.21.61Z"/></svg> 8.3</span>
