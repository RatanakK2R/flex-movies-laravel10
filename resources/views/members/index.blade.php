			...
			<div class="row">
				...
				<div class="col-12">
					<div class="main__title">
						...
						<span class="main__title-stat">{{ count($members) }}  total</span>
						...
						<div class="main__title-wrap">
							...
							<div class="filter" id="filter__sort">
								...
								<div class="filter__item-btn dropdown-toggle" role="navigation" id="filter-sort" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
									<input type="button" value="{{ request()->input('sort', 'Date created') }}">
									<span></span>
								</div>

								<ul class="filter__item-menu dropdown-menu scrollbar-dropdown" aria-labelledby="filter-sort">
									<li><a href="{{ route('members.index', ['sort' => 'created_at']) }}">Date created</a></li>
								</ul>
							</div>
							<!-- end filter sort -->

							<!-- search -->
							<form action="{{ route('members.create') }}" method="GET" class="main__title-form">
								<input type="text" name="search" placeholder="Find movie / tv series.." value="{{ request()->input('search') }}">
								<button type="submit">
						...
						<table class="main__table">
                            ...
                            <tbody>
								<tr>
									...
									<td>
										<div class="main__table-text">{{ $member->lastname }}</div>
									</td>
									<td>
										<div class="main__table-text">{{ $member->position }}</div>
									</td>
									{{-- <td>
									...
									<td>
										<div class="main__table-text">{{ $member->created_at->format('d M Y') }}</div>
									</td>
									<td>
										<div class="main__table-btns">
                                            <a href="#modal-status-{{ $member->id }}" class="main__table-btn main__table-btn--banned open-modal">
											...
											<a href="{{ route('movies.edit', $member->id) }}" class="main__table-btn main__table-btn--edit">
...
{{--											</form>--}}
    ...
    @foreach ($members as $member)
        <div id="modal-status-{{ $member->id }}" class="zoom-anim-dialog mfp-hide modal">
            <h6 class="modal__title">Status Change</h6>

            <p class="modal__text">Are you sure you want to change the status of this movie?</p>

            <form class="modal__btns" action="{{ route('movies.updateStatus', ['id' => $member->id]) }}" method="POST">
                @csrf
                @method('PATCH')
	...
	<div id="modal-delete" class="zoom-anim-dialog mfp-hide modal">
        ...
        <form class="modal__btns" action="{{ route('members.destroy', $member->id) }}" method="POST">
            @csrf
            @method('DELETE')
